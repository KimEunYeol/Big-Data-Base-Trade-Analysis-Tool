...
var dir = document.URL.substring(document.URL.lastIndexOf('/')+1, document.URL.length);
...
$(function() {
  if(dir == 'interest.html') {
    $.getJSON('http://ip-api.com/json?callback=?', function(data) {
      $.getJSON('https://maps.googleapis.com/maps/api/geocode/json?latlng=' + data.lat + ',' + data.lon + '&key=AIzaSyBwKz0B9gmFh21f9pcWXi9SStJeVxoBF0w&language=ko&location_type=ROOFTOP', function(geo) {
        splitted_geo = geo.results[0].formatted_address.split(' ');

        if(splitted_geo[1] != '서울특별시') {
          $('#intWide').val(splitted_geo[1]).attr('selected', 'selected');
//          $('#intWide').val(splitted_geo[1]);
//alert(splitted_geo[1]);
//          $('#intWide option[value=' + splitted_geo[1] + ']').attr('selected', 'selected');
          $('#intCity').find('option').remove();
          cityObj[$('#intWide').val()].index.forEach(function(item) {
            $('#intCity').append('<option value="' + item + '">' + item + '</option>');
          });
        }

        $('#intCity').val(splitted_geo[2]).attr('selected', 'selected');
      });
    });
  }
