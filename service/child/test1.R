...
geo = paste(lon,",",lan,",10km",sep="") 
for(i in 1:length(category)){
  
  x=enc2utf8(category[i])
  x_search=searchTwitter(x,since=date1,geocode=geo,n=10)
...
f<-function(word){
  ...
  for(i in 1:length(word)){
    ...
    colnames(F_search_count)=c("단어","빈도수")
    
    LIST = list("감정점수"=frame,"단어빈도수"=F_search_count)
    F_List<-toJSON(LIST)
